...
| **CLASP_JSON** | （GAS 用）本機 **`.clasp.json` 檔案內容**（不是 Web App 網址） | 單行 JSON，例：`{"scriptId":"你的 Apps Script 專案 ID","rootDir":"./gas"}`。若 CI 出現 `invalid character 'x' at 1:3`，多半是誤貼成 **https://script.google.com/...** 網址，請改貼 .clasp.json 的 JSON 內容。 |
